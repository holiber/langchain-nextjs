...
import {consolidateMarkdownFiles, defaultAgentPrefix} from "@/app/api/chat/myagents/docs-retriver";
...
function replaceCurlyBracesWithUnicode(str: string) {
...
export async function POST(req: NextRequest) {
  ...
  const PREFIX_TEMPLATE = replaceCurlyBracesWithUnicode(consolidateMarkdownFiles(fs, path)) +
 `
Some helpful information you may need:
- Directus GraphQL access_token = LBeZAtOuL2IiWGLnxrgE2LVjrll1zJki
- Directus GraphQL url1 = https://cardiff.alexn.tech/graphql/system // for requesting system endpoints like collections, fields
- Directus GraphQL url2 = https://cardiff.alexn.tech/graphql  // for requesting content created by user
`
  ...
  try {
    ...
    const tools = [new Calculator(), new SerpAPI(), createGraphqlTool()];
